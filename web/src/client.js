...
if (localStorage.getItem('token') === null) {
  pb.NewPlayground({}, { prefix: '/api/twirp' })
    .then(res => {
      localStorage.setItem('token', res.token);
      var msg = `please transfer more than ${res.value.toFixed(
        3
      )} test ether to the deployer account ${res.address} for next step`;
  ...
  if (localStorage.getItem('target') === null) {
    pb.DeployContract(
      {},
      { headers: { authorization: token }, prefix: '/api/twirp' }
    )
      .then(res => {
